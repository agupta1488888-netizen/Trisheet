...
export function TrustSection() {
  return (
    ...
    >
      {POINTS.map(({ icon: Icon, badge, title, body }) => (
        <div
          key={title}
          className="flex flex-col gap-4 rounded-2xl border border-slate-100 bg-white p-6 shadow-lg shadow-slate-200/60 transition-shadow hover:shadow-xl motion-reduce:transition-none"
        >
          <span
            className={`flex size-11 items-center justify-center rounded-full ${badge} shadow-md`}
          >
            <Icon aria-hidden="true" strokeWidth={2} className="size-5 text-white" />
          </span>
